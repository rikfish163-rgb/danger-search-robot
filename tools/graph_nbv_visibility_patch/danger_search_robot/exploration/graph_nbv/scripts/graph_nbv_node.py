...
class GraphNBVNode:
    ...
    def __init__(self) -> None:
        ...
        self.visibility_evaluation_limit = int(
            rospy.get_param("~visibility_evaluation_limit", 120)
        )
    ...
    def build_map_layers(
        ...
        # Dilated occupied cells are used only for visibility ray casting.
        # This prevents rays leaking through a one-cell wall corner or tiny
        # raster gaps and counting UNKNOWN on the other side of a wall.
        wall_dilation_cells = max(
            1,
            int(round(
        ...
        wall_kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE,
            (
                2 * wall_dilation_cells + 1,
                2 * wall_dilation_cells + 1,
            ),
        )
        visibility_blocked = cv2.dilate(
            occupied,
            wall_kernel,
            iterations=1,
        ).astype(np.uint8)

        # These circular gains are only a cheap pre-filter. They are not used
        # as final NBV information gain. Final gain is calculated by ray
        # casting and stops at the first wall.
        ...
        approximate_unknown_count = cv2.filter2D(
            unknown.astype(np.float32),
            cv2.CV_32F,
            info_kernel,
            borderType=cv2.BORDER_CONSTANT,
        )
        approximate_unknown_gain = (
            approximate_unknown_count
            * self.resolution
            * self.resolution
        )
        ...
        approximate_frontier_gain = cv2.filter2D(
        ...
        return {
            ...
            "approximate_unknown_gain": approximate_unknown_gain,
            "approximate_frontier_gain": approximate_frontier_gain,
            "coverage_distance": coverage_distance,
        }

    def compute_visible_information(
        self,
        row: int,
        col: int,
        layers: Dict[str, np.ndarray],
    ) -> Tuple[float, float]:
        """Return wall-occlusion-aware UNKNOWN area and frontier count."""
        visible_unknown = set()
        visible_frontier = set()

        ray_count = max(16, self.visibility_ray_count)
        step_cells = max(
            self.visibility_ray_step / self.resolution,
            0.5,
        )
        max_range_cells = self.information_radius / self.resolution
        max_unknown_cells = max(
            1,
            int(round(
        ...
        for ray_index in range(ray_count):
            ...
            delta_col = math.cos(angle)
            delta_row = math.sin(angle)
            distance_cells = step_cells
            last_cell = None
            unknown_cells_on_ray = 0

            while distance_cells <= max_range_cells:
                current_row = int(round(
                    row + delta_row * distance_cells
                ))
                current_col = int(round(
                    col + delta_col * distance_cells
                ))
                ...
                if (
                    current_row < 0
                    or current_row >= self.map_height
                    or current_col < 0
                    or current_col >= self.map_width
                ):
                    break

                if layers["visibility_blocked"][
                    current_row, current_col
                ]:
                    break

                flat_index = (
                    current_row * self.map_width + current_col
                )

                if layers["frontier"][current_row, current_col]:
                    visible_frontier.add(flat_index)

                if layers["unknown"][current_row, current_col]:
                    visible_unknown.add(flat_index)
                    unknown_cells_on_ray += 1
                    if unknown_cells_on_ray >= max_unknown_cells:
                        break
        ...
        return unknown_area, float(len(visible_frontier))
    ...
    def sample_viewpoints(
        ...
        for row in range(
            ...
            for col in range(
                ...
                approximate_unknown = float(
                    layers["approximate_unknown_gain"][row, col]
                )
                approximate_frontier = float(
                    layers["approximate_frontier_gain"][row, col]
                )

                if approximate_unknown <= 0.0:
                    continue

                pre_candidates.append(
                    (
                        approximate_unknown,
                        approximate_frontier,
        ...
        # Circular gain is allowed only to decide which candidates deserve
        # expensive ray casting. It never becomes the final NBV score.
        pre_candidates.sort(
            key=lambda item: (item[0], item[1], item[2]),
            reverse=True,
        )
        pre_candidates = pre_candidates[
            : self.visibility_evaluation_limit
        ]

        viewpoints: List[Viewpoint] = []

        for (
            _approximate_unknown,
            _approximate_frontier,
            clearance,
            row,
            col,
            x,
            ...
            visible_unknown_gain, visible_frontier_gain = (
                self.compute_visible_information(
                    row,
                    col,
                    layers,
                )
            ...
            if visible_unknown_gain < self.min_unknown_gain_m2:
                continue
            if (
                visible_frontier_gain
            ...
            viewpoints.append(
                Viewpoint(
                    ...
                    y=y,
                    clearance=clearance,
                    unknown_gain=visible_unknown_gain,
                    frontier_gain=visible_frontier_gain,
                    revisit=revisit,
                )
            )
    ...
    def score_viewpoints(
        ...
        for viewpoint in viewpoints:
            ...
            wall_proximity = max(
                0.0,
                self.preferred_clearance - viewpoint.clearance,
            )
    ...
    def send_goal(
        ...
        rospy.loginfo(
            ...
            "utility=%.2f visible_unknown=%.2f m2 visible_frontier=%.0f "
