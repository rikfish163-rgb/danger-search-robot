...
class HybridExplorationNode:
    ...
    def __init__(self) -> None:
        ...
        # 走廊优先任务约束：
        # 节点启动位置应位于走廊入口，机器人朝向走廊深处。
        self.start_exclusion_radius = float(
            rospy.get_param("~start_exclusion_radius", 2.50)
        )
        self.min_forward_progress = float(
            rospy.get_param("~min_forward_progress", -0.30)
        )
        self.progress_reward_weight = float(
            rospy.get_param("~progress_reward_weight", 2.50)
        )
    ...
    def mark_trajectory(self, pose: Tuple[float, float, float]) -> None:
        ...
        first_pose = self.mission_origin is None
        if first_pose:
            self.mission_origin = (x, y, yaw)
            rospy.loginfo(
                "[hybrid_exploration] corridor mission origin "
                "x=%.2f y=%.2f yaw=%.1f deg",
                x,
                y,
                math.degrees(yaw),
            )
        ...
        with self.map_lock:
            ...
            # 前厅不属于今天的重点搜索区。
            # 启动时直接把起点周围较大范围标记为已经覆盖。
            if first_pose:
                start_cells = max(
                    1,
                    int(round(
                        self.start_exclusion_radius
                        / self.map_resolution
                    )),
                )
                cv2.circle(
    ...
    def generate_candidates(
        ...
        # 计算每个栅格相对任务起点的前向进度。
        # 启动位置之后的区域为正，前厅方向为负。
        progress_grid = np.zeros(
            grid_c.shape,
            dtype=np.float32,
        )

        if mission_origin is not None:
            mission_x, mission_y, mission_yaw = mission_origin

            rows, cols = np.indices(grid_c.shape)

            full_rows = rows + r0
            full_cols = cols + c0

            local_x = (full_cols + 0.5) * resolution
            local_y = (full_rows + 0.5) * resolution
            ...
            heading_x = math.cos(mission_yaw)
            heading_y = math.sin(mission_yaw)

            progress_grid = (
                heading_x * (world_x - mission_x)
                + heading_y * (world_y - mission_y)
            ).astype(np.float32)

            # 忽略机器人启动方向后方的前厅区域。
            safe = (
                safe
                & (progress_grid >= self.min_forward_progress)
            )

        # 用于 frontier 检测的墙体预处理：
        # 封闭墙体中 1～2 个栅格的小孔，避免墙角 UNKNOWN 缝隙
        # 被误认为值得探索的 frontier。真实门口不会被 3x3 核封死。
        occupied_for_frontier = (
            grid_c >= self.occupied_threshold
        ).astype(np.uint8)

        wall_kernel = np.ones((3, 3), dtype=np.uint8)
        ...
        for label in range(1, count):
            ...
            progress = float(progress_grid[local_r, local_c])
            ...
            score = (
                ...
                + self.progress_reward_weight * max(0.0, progress)
            )
    ...
    def handle_goal_terminal(
        ...
        if state == GoalStatus.SUCCEEDED:
            ...
            if goal_type == "frontier":
                ...
                # 到达一个 frontier 后，暂时屏蔽其周围区域。
                # 防止墙角或细长 UNKNOWN 缝隙不断产生相邻目标。
                self.add_blacklist(candidate["x"], candidate["y"])
