...
void State_RL::infer_thread_callback()
{
    while(infer_thread_runnning == State_RL::RUNNING)
    {
        ...
        inputs.push_back(flattened_obs.to(torch::kCPU));
...
void State_RL::updateCommandTensor(){
    ...
    commands_tensor[0] = finiteAxis(this->current_cmd_vel_.linear_x / _keyboardVxScale);
    commands_tensor[1] = finiteAxis(this->current_cmd_vel_.linear_y / _keyboardVyScale);
    commands_tensor[2] = finiteAxis(this->current_cmd_vel_.angular_z / _keyboardWzScale);
}
...
void State_RL::refresh_rl_obs(){
    ...
    if (real == false)
    {
        ...
        obs_history_tensor = torch::cat({
            obs_history_tensor.slice(0, 1, HISTORY_LEN).to(torch::kCPU),  // 删除最早的一步
            obs_tensor.unsqueeze(0).to(torch::kCPU)  // 将当前 obs_tensor 插入到历史中
        }, 0);  // 按行（第0维）拼接
    }
    else if (real == true)
    {
