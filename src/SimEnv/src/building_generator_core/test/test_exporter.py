...
class ExporterTest(unittest.TestCase):
    ...
    def test_dynamic_door_initial_link_pose_matches_initial_open(self) -> None:
        constraints = BuildingConstraints.from_dict(
            {
                "seed": 19,
                "floor_count": 2,
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_by_id = {item["id"]: item for item in door_config["doors"]}
            ...
            for door in layout.door_specs:
                ...
                model = next(
                    ...
                    if model.get("name") == f"dynamic_{door.id}"
                )
                config = config_by_id[door.id]
                pose_key = "left_open" if door.initial_open else "left_closed"
                expected_left = config["panel_poses"][pose_key]
                pose_key = "right_open" if door.initial_open else "right_closed"
                expected_right = config["panel_poses"][pose_key]

                for link_name, expected in (
                    ("left_panel", expected_left),
                    ("right_panel", expected_right),
                ):
                    actual = [
                        float(value)
                        for value in model.find(f"link[@name='{link_name}']").findtext("pose").split()
                    ]
                    self.assertEqual(len(actual), len(expected))
                    for actual_value, expected_value in zip(actual, expected):
                        self.assertAlmostEqual(actual_value, float(expected_value), places=6)
    ...
    def test_main_entrance_open_panels_clear_the_doorway(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            entrance = next(
                item
                for item in door_config["doors"]
                if item["kind"] == "main_entrance"
            )

            panel_width = entrance["width"] / 2.0
            required_offset = entrance["width"] / 2.0 + panel_width / 2.0
            self.assertGreaterEqual(
                abs(entrance["panel_poses"]["left_open"][1]),
                required_offset,
            )
            self.assertGreaterEqual(
                abs(entrance["panel_poses"]["right_open"][1]),
                required_offset,
            )
